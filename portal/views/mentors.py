...
def mentor_applications_index(request: HttpRequest) -> HttpResponse:
    ...
    if "target_semester" in context:
        context["pending_applications"] = context[
            "target_semester"
        ].mentor_applications.filter(is_accepted__isnull=True)
        context["accepted_applications"] = context[
            "target_semester"
        ].mentor_applications.filter(is_accepted=True)
        context["denied_applications"] = context[
            "target_semester"
        ].mentor_applications.filter(is_accepted=False)

        context["tags_with_counts"] = ProjectTag.objects.annotate(
            pending_application_count=Count(
                "mentor_applications",
                filter=Q(
                    mentor_applications__is_accepted__isnull=True,
                    mentor_applications__semester=context["target_semester"],
                ),
            ),
            accepted_application_count=Count(
                "mentor_applications",
                filter=Q(
                    mentor_applications__is_accepted=True,
                    mentor_applications__semester=context["target_semester"],
                ),
            ),
...
class MentorApplicationView(SuccessMessageMixin, LoginRequiredMixin, CreateView):
    form_class = MentorApplicationForm
    template_name = "portal/mentors/application.html"
    success_message = (
        "Your application has been submitted and will be reviwed "
